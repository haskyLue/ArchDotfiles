...
class TmuxBar:
    ...
    def __update_network_interface(self):
        self.__network_interface = sh.awk( sh.head(sh.netstat("-nr"), '-n5'), '/default/ {print $6}').strip()
    ...
    def wifi(self):
        ...
        if(self.__network_interface):
            mSsid = sh.awk(
                    sh.networksetup("-getairportnetwork",self.__network_interface),
                    '{print $4}').strip()
            mIp = sh.awk(
    ...
    def batt(self):
        return sh.awk(
                sh.pmset('-g','batt'),
                '/InternalBattery/ {print $3$4}').replace(";",' ').strip()

    def traffic(self):
        def __trafficPerSecond():
            self.__flow_new = sh.awk(sh.netstat("-I",self.__network_interface,"-bn"),'NR==2 {print $7,$10}').strip().split(" ")
        ...
        if(self.__network_interface):
            ...
            return "%s ⇲ %.1fK ⇱ %.1fK" % (self.__network_interface,rate[0],rate[1])
        else:
            return "0.0K"
    ...
    def memory(self):
        mAvailMem = sh.awk(sh.memory_pressure(),'/percent/ {print $5}').strip()
        mTopProcess = re.sub("\s+"," ",
                sh.top("-l","1","-o","mem","-U","hasky","-n1","-stats","COMMAND,MEM").split("\n")[-2])

        return mAvailMem + " <" + mTopProcess + ">"

    def disk(self):
        mAvailMount = sh.awk('NR!=1 {print $5}', _in=sh.df("-lh")).replace("\n"," ").strip()
        return  mAvailMount 

    # 这里就不检查参数合法性了
    def parse(color,content,label):
        return TmuxBar.__tmux_color_tmpl.replace(
                "TMUX_COLOR",color.upper()).replace(
                        "TMUX_CONTENT",content).replace(
                                "TMUX_LABEL",label)
